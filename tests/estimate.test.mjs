...
test('provider success is validated and failures never become fabricated estimates',async()=>{
  ...
  try{
    globalThis.fetch=async(url,options)=>{assert.equal(url,'https://api.anthropic.com/v1/messages');const body=JSON.parse(options.body);assert.equal(body.model,'claude-opus-5');assert.ok(body.system.includes('untrusted'));assert.ok(body.system.includes('never add an object that was not provided'),'items anti-invention rule must stay in the prompt');assert.ok(body.max_tokens>=4000,'adaptive thinking shares max_tokens with the answer');return Response.json({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify(estimate)}]});};
