...
The main website now links to the separate `/register/` questionnaire. Form markup is generated by `registration-page.cjs`; behavior is in `assets/registration.js`. New leads use `customer_use.kind=demand_validation_v2` and store use cases, size, timing/date, duration, optional budget, quoted price/response when configured, readiness, contact preference, optional LINE ID/email and source/campaign/ad attribution. Contact details come last. Price configuration is currently null pending Newton's list. See DEMAND_V2_2026-09-15.md for the complete contract and qualification rules.

The owner dashboard has a dedicated V2 intent section with timing, size, qualification, price acceptance and readiness metrics. CAC requires matching manual ad spend and explicit paid-medium tags. Requested demand statuses are persisted in the existing follow-up notes envelope, retaining the constrained legacy status field and audit history. Public permissions and existing records are unchanged.

Live deployment fc32942 verified on September 15. One real marked acceptance submission, `TEST ONLY DEMAND V2 20260915` with source `setup_test_demand_v2_20260915`, reached the database-confirmed success view. Budget/email/price were left empty; contact preference Phone, readiness researching. The normal dashboard excludes this record. No LINE or Facebook message was sent.

The earlier sections below describe legacy records and prior versions where noted.
