...
$('demandForm').onsubmit=async event=>{
 ...
 const ok=await (window.saveQuestionnaireWithPhotos?window.saveQuestionnaireWithPhotos(row):sbInsert('leads',row));busy=false;for(const id of ['submitBtn','backBtn','langBtn'])$(id).disabled=false;
