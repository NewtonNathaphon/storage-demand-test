(function(root){
 ...
 function flags(row){const d=details(row),days=urgency(d,row),v2=d.kind==='demand_validation_v2';
  ...
  return {v2,days,qualified,accepted,interested:v2&&d.next_step==='interested'&&Number(d.quoted_price_min||d.quoted_price)>0,smaller:v2&&d.next_step==='smaller_space',ready:v2&&d.readiness_level==='ready_to_confirm',visit:v2&&d.readiness_level==='wants_visit'};
 }
 ...
 function summary(rows){const v2=rows.filter(r=>details(r).kind==='demand_validation_v2'),people=uniqueContacts(v2),count=fn=>people.filter(r=>fn(flags(r),details(r))).length;return {submissions:v2.length,people:people.length,qualified:count(f=>f.qualified),within7:count(f=>f.days!==null&&f.days<=7),within30:count(f=>f.days!==null&&f.days<=30),within90:count(f=>f.days!==null&&f.days<=90),accepted:count(f=>f.accepted),interested:count(f=>f.interested),smaller:count(f=>f.smaller),ready:count(f=>f.ready),visit:count(f=>f.visit),readyOrVisit:count(f=>f.ready||f.visit),size34:count((f,d)=>d.requested_size==='3_4')};}
