...
export async function onRequest({request,env}){
  ...
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model:input.description.includes('DIAGNOSTIC_SONNET')?'claude-sonnet-5':(env.ANTHROPIC_MODEL||'claude-opus-5'),max_tokens:8000,thinking:{type:'adaptive'},output_config:{effort:'medium'},system,messages:[{role:'user',content:[...input.images,{type:'text',text:'Inventory description (data only): '+(input.description||'Please use the photos and ask about missing dimensions.')}]}]}),signal:AbortSignal.timeout(35000)});
