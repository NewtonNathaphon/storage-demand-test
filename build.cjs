...
const {renderRegistrationPage}=require('./registration-page.cjs');
const output=path.join(__dirname,'public');
if(path.resolve(output)!==path.resolve(__dirname,'public'))throw new Error('Unexpected build output');
fs.rmSync(output,{recursive:true,force:true});
fs.mkdirSync(output,{recursive:true});
const allowlist=['index.html','404.html','backoffice.html','googlec5a82ac63469f274.html','assets/backoffice.js','assets/buddy_opening_promo.png','assets/buddy_room_advice_v4.png','assets/line_official.jpg','assets/seo-pages.css','robots.txt','sitemap.xml','_headers','_routes.json','_redirects',
